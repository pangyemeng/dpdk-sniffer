...
static void app_init_mbuf_pools(void)
{
	...
	/* 每个物理CPU创建一个内存池，防止跨核访问，在NUMA结构下性能有提升。 */
	for (socket = 0; socket < APP_MAX_SOCKETS; socket++)
	{
		...
		app.pools[socket] = rte_pktmbuf_pool_create(name, APP_DEFAULT_MEMPOOL_BUFFERS, APP_DEFAULT_MEMPOOL_CACHE_SIZE, 0, APP_DEFAULT_MBUF_DATA_SIZE, socket);
		if (app.pools[socket] == NULL)
		{
			rte_panic("Cannot create mbuf pool on socket %u\n", socket);
		}
	}
	//初始化每个逻辑核是否启用以及关联到同一个物理逻辑核上
	for (lcore = 0; lcore < APP_MAX_LCORES; lcore++)
	{
		if (app.lcore_params[lcore].type == e_APP_LCORE_DISABLED)
		{
...
static void app_init_rings_rx(void)
{
	...
	/* 为每个接收包侧初始化环形队列 ,检查每个逻辑核作用 */
	for (lcore = 0; lcore < APP_MAX_LCORES; lcore++)
	{
		struct app_lcore_params_io *lp_io = &app.lcore_params[lcore].io;
		unsigned socket_io, lcore_flow;

		//仅初始化逻辑核角色为e_APP_LCORE_IO的
		if ((app.lcore_params[lcore].type != e_APP_LCORE_IO) || (lp_io->rx.n_nic_queues == 0))
		{
			continue;
		}
		...
		for (lcore_flow = 0; lcore_flow < APP_MAX_LCORES; lcore_flow++)
		{
			...
			printf("Creating ring to connect I/O lcore %u (socket %u) with flow lcore %u ...\n", lcore, socket_io, lcore_flow);
			snprintf(name, sizeof(name), "app_ring_rx_s%u_io%u_flow%u", socket_io, lcore, lcore_flow);
...
#define MAX_CHECK_TIME 90 /* 9s (90 * 100ms) in total */
	...
	for (count = 0; count <= MAX_CHECK_TIME; count++)
	{
		...
		for (portid = 0; portid < port_num; portid++)
		{
			...
			if (n_rx_queues == 0) continue;
			memset(&link, 0, sizeof(link));
			rte_eth_link_get_nowait(portid, &link);
			/* print link status if flag set */
			if (print_flag == 1)
			{
				if (link.link_status) printf("Port %d Link Up - speed %u "
...
static void app_init_nics(void)
{
	...
	for (port = 0; port < APP_MAX_NIC_PORTS; port++)
	{
		...
		for (queue = 0; queue < APP_MAX_RX_QUEUES_PER_NIC_PORT; queue++)
		{
			...
			ret = rte_eth_rx_queue_setup(port, queue, (uint16_t) app.nic_rx_ring_size, socket, NULL, pool);
			if (ret < 0)
			{
